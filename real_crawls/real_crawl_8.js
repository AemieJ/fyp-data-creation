...
function addToCSV(titles, texts) {
    let filePath = "../real/real_fortune_content.csv";
...
function addToCSVNoTexts(titles, dates, links) {
    let filePath = "../real/real_fortune_title.csv";
    let log = csvWriter({sendHeaders: false});
    if (!fs.existsSync(filePath))
...
function grabText() {
    let lst_ = [];
    document.querySelectorAll(".gutenbergContent__content--1FgGp > p").forEach((el) => {
        text = el.innerText; 
        if (text.length != 0) { 
            lst_.push(text)
        }
...
function detailsExtract() {
    ...
    document.querySelectorAll(".termArchiveContentListItem__title--14jcP > a").forEach((el) => {
        titles.push(el.innerText);
        links.push(el.href);
    });

    document.querySelectorAll(".termArchiveContentListItem__meta--3RUO6 > p").forEach((el) => {
...
async function paginationAndScrape(totalPages) {
    ...
    for (let page_ = 1; page_ < 534; ++page_) {
        console.log(`Scraping through page: ${page_ + 1}`);
        let from_ = page_ * size; 
        let URL = `https://fortune.com/section/health/page/${page_ + 1}/`;
...
async function extractPostDetails() {
	try {
		const URL = 'https://fortune.com/section/health/';
		const browser = await puppeteer.launch();
		const page = await browser.newPage();
