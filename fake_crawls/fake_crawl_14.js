...
function addToCSV(titles, texts) {
    let filePath = "../fake/fake_newsbiscuit_content.csv";
...
function addToCSVNoTexts(titles, dates, links) {
    let filePath = "../fake/fake_newsbiscuit_title.csv";
    let log = csvWriter({ sendHeaders: false });
    if (!fs.existsSync(filePath))
...
function detailsExtract() {
    ...
    document.querySelectorAll("article > div:nth-child(2) h3 a").forEach((el) => {
        titles.push(el.innerText);
        links.push(el.href);
    });

    document.querySelectorAll("article > div:nth-child(2) time.date").forEach((el) => {
...
async function paginationAndScrape() {
    ...
    for (let page_ = 1; page_ < 3; ++page_) {
        console.log(`Scraping through page: ${page_ + 1}`);
        let from_ = page_ * size;
        let URL = `http://www.newsbiscuit.com/page/${page_ + 1}/?s=healthcare`;
...
async function extractPostDetails() {
    const URL = 'http://www.newsbiscuit.com/?s=healthcare';
    const browser = await puppeteer.launch();
    const page = await browser.newPage();
