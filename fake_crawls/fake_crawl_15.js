...
function addToCSV(titles, texts) {
    let filePath = "../fake/fake_fmobserver_content.csv";
...
function addToCSVNoTexts(titles, dates, links) {
    let filePath = "../fake/fake_fmobserver_title.csv";
    let log = csvWriter({ sendHeaders: false });
    if (!fs.existsSync(filePath))
...
function grabText() {
    let lst_ = [];
    document.querySelectorAll("div.single-archive span strong").forEach((el, idx) => {
        text = el.innerText;
        if (idx !== 0) {
            if (text.length !== 0) {
                lst_.push(text)
...
function detailsExtract() {
    ...
    document.querySelectorAll("h2.bl2page-title a").forEach((el) => {
        titles.push(el.innerText);
        links.push(el.href);
    });

    document.querySelectorAll("div.bl2page-info").forEach((el) => {
...
async function paginationAndScrape() {
    ...
    for (let page_ = 1; page_ < 2; ++page_) {
        ...
        let URL = `https://fmobserver.com/page/${page_ + 1}/?s=healthcare`;
...
async function extractPostDetails() {
    const URL = 'https://fmobserver.com/?s=healthcare';
    const browser = await puppeteer.launch();
    const page = await browser.newPage();
