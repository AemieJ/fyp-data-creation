...
// World news daily report - mental illness [no publishing dates - fill n/a [imputation]]
...
async function paginationAndScrape() {
    ...
    for (let page_ = 1; page_ < 2; ++page_) {
        ...
        let URL = `https://worldnewsdailyreport.com/category/mental-illness-2/page/${page_ + 1}/`;
...
async function extractPostDetails() {
    const URL = 'https://worldnewsdailyreport.com/category/mental-illness-2/';
    const browser = await puppeteer.launch();
    const page = await browser.newPage();
