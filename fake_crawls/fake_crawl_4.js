...
function addToCSV(titles, texts) {
    let filePath = "../fake/fake_prntly_content.csv";
...
function addToCSVNoTexts(titles, dates, links) {
    let filePath = "../fake/fake_prntly_title.csv";
    let log = csvWriter({ sendHeaders: false });
    if (!fs.existsSync(filePath))
...
function detailsExtract() {
    ...
    document.querySelectorAll("article div.entry-content-holder h2 a").forEach((el) => {
        titles.push(el.innerText);
        links.push(el.href);
    });

    document.querySelectorAll(".entry-content-holder time.entry-date").forEach((el) => {
...
async function paginationAndScrape() {
    ...
    for (let page_ = 1; page_ < 11; ++page_) {
        console.log(`Scraping through page: ${page_ + 1}`);
        let from_ = page_ * size;
        let URL = `https://prntly.com/page/${page_+1}/?s=health`;
...
async function extractPostDetails() {
    const URL = 'https://prntly.com/?s=health';
    const browser = await puppeteer.launch();
    const page = await browser.newPage();
